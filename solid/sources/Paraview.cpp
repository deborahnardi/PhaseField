...
void FEM::showResults(int _nStep)
{
    ...
    */
    if (rank == 0)
    {
        ...
        std::fstream filename(r1.c_str(), std::ios_base::out);

        std::string r2 = resultsPath + "results/hdf5/FEM_" + name + "_connectivity.h5";
        if (_nStep == 0)
            std::fstream filename2(r2.c_str(), std::ios_base::out);
        ...
        */
        ...
        hsize_t scalarDims[2] = {numNodes, 1};               // Dimensions of the scalar dataset is 1D, numNodes
        ...
        */
        ...
        for (auto n : discritizedNodes)
            scalar[n->getIndex()] = n->getDOF(2)->getValue();
